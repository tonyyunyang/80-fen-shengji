...
export function apiSeatFields(seat,index,profiles,seatLabel){
  ...
  return `<div class="provider-options"><label for="provider-${index}">${pick('API 连接','API connection')}</label><select id="provider-${index}" aria-label="${escape(seatLabel)} · ${pick('API 连接','API connection')}">`+
    ...
    (seat.provider==='mock'?'':profile?.sponsored?`<small class="connection-note">${pick('Tony 请客，无需填写 key。额度用完或模型超时时，由免费陪练接手。','AI is on Tony; no key needed. When an allowance or timeout is reached, free practice bots take over.')}</small>`:`<div class="seat-connection-route"><button class="secondary" type="button" data-manage-connection="${escape(profile?.id||'')}" data-api-seat="${index}">${profile?profile.active?pick('管理这条连接','Manage this connection'):pick('启用 / 管理连接','Enable / manage connection'):pick('＋ 添加 API 连接','＋ Add API connection')} ↗</button><small>${profile?profile.active?pick('需要更换服务或 key，可直接在这里管理。','Manage this service or its key here.'):pick('在这里填入 key 并读取模型，完成后回到这一席。','Enter a key and read models here, then return to this seat.'):pick('点击这里添加，也可使用右侧「API 连接」。','Add it here, or use API connections in the side panel.')}</small></div>`)+
