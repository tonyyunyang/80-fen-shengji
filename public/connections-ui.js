...
export function apiSeatFields(seat,index,profiles,seatLabel,capabilities={}){
  ...
  return `<div class="provider-options"><label for="provider-${index}">${pick('API 连接','API connection')}</label><select id="provider-${index}" aria-label="${escape(seatLabel)} · ${pick('API 连接','API connection')}">`+
    ...
    profiles.map(p=>option(p.id,p.name+(p.sponsored?pick(' · Tony 提供',' · sponsored'):p.active?pick(' · 已启用',' · enabled'):pick(' · 需要 key',' · needs a key')),current)).join('')+
    option('mock',pick('离线模拟 · 不调用模型','Offline simulation · no model'),current)+'</select>'+
    (seat.provider==='mock'?'':!profile&&capabilities?.personalConnections===false?`<small class="connection-note">${pick('网站 AI 尚未启用，可以使用免费陪练。','Hosted AI is not enabled yet. Free practice bots are available.')}</small>`:profile?.sponsored?`<small class="connection-note">${pick('Tony 请客，无需填写 key。额度用完或模型超时时，由免费陪练接手。','AI is on Tony; no key needed. When an allowance or timeout is reached, free practice bots take over.')}</small>`:`<div class="seat-connection-route"><button class="secondary" type="button" data-manage-connection="${escape(profile?.id||'')}" data-api-seat="${index}">${profile?profile.active?pick('管理这条连接','Manage this connection'):pick('启用 / 管理连接','Enable / manage connection'):pick('＋ 添加 API 连接','＋ Add API connection')} ↗</button><small>${profile?profile.active?pick('需要更换服务或 key，可直接在这里管理。','Manage this service or its key here.'):pick('在这里填入 key 并读取模型，完成后回到这一席。','Enter a key and read models here, then return to this seat.'):pick('点击这里添加，也可使用右侧「API 连接」。','Add it here, or use API connections in the side panel.')}</small></div>`)+
    (profile?.sponsored&&capabilities?.personalConnections!==false?`<button class="secondary" type="button" data-manage-connection="" data-previous-connection="${escape(profile.id)}" data-api-seat="${index}">${pick('使用自己的 API','Use your own API')} ↗</button>`:'')+
...
export function setupConnectionsDialog({state,seatLabel,post,refresh,notify,onLinked=()=>{}}){
  ...
  const personal=()=> (state()?.connections||[]).filter(p=>!p.sponsored);
  ...
  function edit(id){
    const p=personal().find(item=>item.id===id);editing=p?.id||null;catalog=new Map((p?.models||[]).map(m=>[m.id,m]));
  ...
  function open(id,seat=null,previous=id){
    if(!busy){sourceSeat=seat;sourceConnection=previous||null;edit(id===undefined?personal()[0]?.id||null:id);}
  ...
  document.addEventListener('click',event=>{const button=event.target.closest('[data-manage-connection]');if(button)open(button.dataset.manageConnection||null,Number(button.dataset.apiSeat),button.dataset.previousConnection??button.dataset.manageConnection);});
