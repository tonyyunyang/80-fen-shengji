...
const $=id=>document.getElementById(id), directions=[t('南'),t('东'),t('北'),t('西')];
const typeName=seat=>{
  if(seat.kind==='human')return t('人类玩家');
  if(seat.kind==='peilian')return t('陪练');
  if(seat.provider==='mock')return t('离线模拟');
  const profile=connectionFor(seat,latest?.connections||[]),model=profile?.models.find(m=>m.id===seat.model);
  return profile?.active&&model?(kimiCodeRequestOptions(seat.model,profile?.baseUrl,512)?pick('Kimi · 快速','Kimi · fast'):(model.label||seat.model))+' · AI':pick('陪练 · API 未启用','Practice bot · API inactive');
};
const defaultSeats=()=>directions.map((direction,index)=>({name:index===0?t('你'):direction+t('家'),kind:index?'peilian':'human',provider:'mock',model:''}));
...
try{const stored=JSON.parse(localStorage.getItem('eighty-config'));if(stored?.seats?.length===4&&stored.seats.every(seat=>seat&&['human','api','peilian'].includes(seat.kind)))config={...config,...stored};const options=JSON.parse(localStorage.getItem('eighty-pixel-options'));appearance=readPreferences(options);}catch{}
for(const seat of config.seats){delete seat.referenceAdvice;if(seat.kind==='api')seat.promptLanguage=seat.promptLanguage==='en'?'en':'zh';}
config.rules={gates:true,fullRebel:'off',speedRun:false,partialTractorFollow:true,...config.rules,firstDealer:'random'};
try{config.limits={...config.limits,timeoutMs:decisionTimeoutMs(config.limits?.timeoutMs)};}catch{config.limits={maxRequests:100,timeoutMs:12000};}
let status={mock:true,openai:false,claude:false,qwen:false},latest=null,viewer=0,stream,selected=new Set(),lastDecision=null,noticeTimer;
let quiz=null,quizKey=null,quizSelection=new Set(),quizAnswered=false;
let arriving=null, recalled=new Map();
...
function option(value,label,current,disabled=false){return '<option value="'+escape(value)+'"'+(value===current?' selected':'')+(disabled?' disabled':'')+'>'+escape(label)+'</option>';}
function renderSetup() {
  ...
  $('setup').innerHTML = config.seats.map((seat, index) =>
    '<div class="seat-form"><div class="seat-letter team-' + index % 2 + '">' + directions[index] + '</div><div><label for="kind-' + index + '">' + (index % 2 ? t('东西搭档') : t('南北搭档')) + '</label><select id="kind-' + index + '" aria-label="' + directions[index] + t('家玩家类型">') +
    option('human', t('人类玩家'), seat.kind) + option('peilian', t('陪练 · 本地策略'), seat.kind) + option('api', t('API 模型'), seat.kind) + '</select></div>' +
    (seat.kind === 'api' ? apiSeatFields(seat, index, latest?.connections || []) : '') + '</div>'
  ).join('') +
    t('<div class="presets"><button class="preset" id="apiPreset">一人 · 三 API</button><button class="preset" id="practicePreset">一人 · 三陪练</button><button class="preset" id="mixedPreset">两席离线模拟</button><button class="preset" id="watchPreset">四席观战</button></div>') +
    t('<details class="options" id="tableOptions"><summary>牌桌设置</summary>') +
    t('<label class="option-row">必打 2 / 5 / 10 / K<input id="gates" type="checkbox"') + (config.rules.gates !== false ? ' checked' : '') + '></label>' +
    t('<label class="option-row">速通阶梯<input id="speedRun" type="checkbox"') + (config.rules.speedRun ? ' checked' : '') + '></label>' +
    t('<label class="option-row">有短拖拉机也须跟<input id="partial" type="checkbox"') + (config.rules.partialTractorFollow !== false ? ' checked' : '') + '></label>' +
    t('<label class="option-row">低分 / 少主重发<select id="rebel">') + option('off', t('关闭'), config.rules.fullRebel) + option('redeal', t('仅重新发牌'), config.rules.fullRebel) + option('scramble', t('重发并重新抢庄'), config.rules.fullRebel) + '</select></label>' +
    t('<label class="option-row">发牌节奏<select id="dealSpeed">') + option('500', t('从容 · 每人每 2 秒一张'), String(config.dealIntervalMs || 500)) + option('700', t('慢速 · 每人每 2.8 秒一张'), String(config.dealIntervalMs || 500)) + '</select></label>' +
    t('<label class="option-row">机器出牌节奏<select id="speed">') + option('600', t('从容 · 600ms'), String(config.speed)) + option('1200', t('慢速 · 1.2s'), String(config.speed)) + option('100', t('快速 · 100ms'), String(config.speed)) + '</select></label>' +
    t('<label class="option-row">本桌 API 请求上限<input id="requestLimit" type="number" min="0" max="2000" value="') + escape(config.limits.maxRequests) + t('"></label><p class="fine">设为 0 时，API 席全部由陪练代打，不发送请求。</p>') +
    t('<label class="option-row">单次输出上限<input id="outputLimit" type="number" min="128" max="4096" value="') + (config.limits.maxOutput || 512) + '"></label>' +
    t('<label class="option-row">API 决策限时（含重试）<input id="timeoutLimit" type="number" min="1" max="') + MAX_DECISION_MS / 1000 + '" value="' + (config.limits.timeoutMs / 1000) + t('" aria-label="API 决策最长等待秒数"></label><p class="fine">最多等待 12 秒，超时由陪练代打本次。下一次仍优先使用 API。</p></details>') +
    (latest?.restoreAvailable && !latest.game ? t('<button class="secondary restore-local" id="restoreLocal">继续此前的本地牌局</button>') : '') +
    '<button class="primary start" id="startGame"' + (!csrfToken ? ' disabled' : '') + '><span>' + (latest?.game ? t('开始新牌桌') : t('开始牌局')) + '</span><span>→</span></button>' +
    '<p class="fine">'+pick('首局随机选庄，亮主只决定主花色。选择好四个座位后再发牌。','The first dealer is chosen randomly; declarations choose the trump suit. Set all four seats before dealing.')+'</p>';
  ...
  config.seats.forEach((seat, index) => {
    if ($('endgame-' + index)) $('endgame-' + index).onchange = event => { config.seats[index].endgameAnalysis = event.target.checked; storeConfig(); };
    if ($('prompt-language-' + index)) $('prompt-language-' + index).onchange = event => { config.seats[index].promptLanguage = event.target.value; storeConfig(); };
    $('kind-' + index).onchange = (event) => { config.seats[index].kind = event.target.value; if (event.target.value === 'api') { const profile=connectionFor(config.seats[index],latest?.connections||[])||latest?.connections?.[0]; const model=profile?.models.some(model=>model.id===config.seats[index].model)?config.seats[index].model:''; config.seats[index].provider=profile?.provider||'qwen'; config.seats[index].connectionId=profile?.id; config.seats[index].model=model; config.seats[index].promptLanguage??='zh'; } storeConfig(); renderSetup(); };
  ...
  $('apiPreset').onclick = () => { const profile=latest?.connections?.[0]; config.seats=defaultSeats().map((seat,index)=>index?{...seat,kind:'api',provider:profile?.provider||'qwen',connectionId:profile?.id,model:''}:seat); storeConfig(); renderSetup(); };
  ...
  $('watchPreset').onclick = () => { config.seats = practiceSeats().map((seat, index) => ({ ...seat, kind: 'peilian', name: directions[index] + t('家') })); storeConfig(); renderSetup(); };
  $('startGame').onclick = () => startMatch();
  if ($('restoreLocal')) $('restoreLocal').onclick = async () => { try { await post('restore-local', {}); await refreshState(); renderSetup(); showScreen('menu'); } catch(error) { notify(error.message); } };
  renderMenu();
}
function storeConfig() { try { localStorage.setItem('eighty-config', JSON.stringify(config)); } catch { notify(t('浏览器未能保存设置，本次仍可使用。')); } }

function showScreen(mode){handDrag?.cancel();viewMode=mode;$('mainMenu').hidden=mode!=='menu';$('settingsView').hidden=mode!=='settings';$('newGameView').hidden=mode!=='setup';$('gameView').hidden=mode!=='game';document.body.dataset.screen=mode;try{sessionStorage.setItem('eighty-screen',mode==='game'?'game':'menu');}catch{}render();presence();}
function applyAppearance(save=false){
  ...
  if(save)try{localStorage.setItem('eighty-pixel-options',JSON.stringify(appearance));}catch{}
  handHover?.refresh();
}
function renderMenu(){
  const game=latest?.game;$('menuStart').disabled=!connected||!csrfToken||starting;
  text('menuStart','新游戏','New game');$('continueGame').hidden=!game;
  text('continueGame',game?.score?'查看本局结果':'继续游戏',game?.score?'View round result':'Continue');
  $('menuSummary').textContent=config.seats.map((seat,index)=>directions[index]+' · '+typeName(seat)).join('  /  ');
  $('resumeSummary').textContent=game?pick('已保留当前对局，可继续或另开一桌。','Your current game is saved. Continue it or start a new table.'):pick('点击新游戏，安排四个座位，一起上桌。','Choose New game, arrange four seats, and take your place.');
  $('connectionLabel').textContent=connected?pick('● 本地牌桌已连接','● Local table connected'):pick('○ 正在连接牌桌…','○ Connecting to the table…');
}
async function startMatch(confirmed=false){
  if(starting||!connected)return;
  if(latest?.game&&!confirmed){$('restartDialog').showModal();return;}
  if(config.seats.some(seat=>seat.kind==='api'&&seat.provider!=='mock'&&(!connectionFor(seat,latest?.connections||[])||!seat.model))){notify(pick('请先为 API 座位连接服务并选择模型，或选择陪练。','Connect a service and choose a model for each API seat, or choose a practice bot.'));return;}
  starting=true;renderMenu();
  try{await post('start',config);viewer=config.seats.findIndex(seat=>seat.kind==='human');selected.clear();quiz=null;quizKey=null;await refreshState();connect();$('restartDialog').close();showScreen('game');}
  catch(error){notify(error.message);}finally{starting=false;renderSetup();renderMenu();}
}
async function resumeGame(){try{if(latest?.paused)await post('pause',{paused:false});$('pauseDialog').close();await refreshState();showScreen('game');}catch(error){notify(error.message);}}
async function mainMenu(){if(latest?.game&&!latest.paused&&connected)try{await post('pause',{paused:true});}catch(error){notify(error.message);}for(const dialog of document.querySelectorAll('dialog[open]'))dialog.close();showScreen('menu');$('continueGame').focus();}
async function pauseGame(){handDrag?.cancel();if(connected)try{if(latest?.game&&!latest.paused)await post('pause',{paused:true});await refreshState();}catch(error){notify(error.message);}$('pauseReason').textContent=connected?pick('返回主菜单可以调整下一场的设置。','Return to the main menu to configure your next game.'):pick('连接暂时中断，正在重连。','The connection is interrupted. Reconnecting.');$('pauseDialog').showModal();}
function presence(){if(!csrfToken)return;fetch('/api/presence',{method:'POST',headers:{'content-type':'application/json','x-eighty-csrf':csrfToken},body:JSON.stringify({clientId,visible:viewMode==='game'&&!document.hidden}),keepalive:true}).catch(()=>{});}
function receive(next){
  if(next.csrf===csrfToken&&next.revision<latest?.revision)return;
  const oldProfiles=JSON.stringify(latest?.connections||[]);csrfToken=next.csrf||'';
  ...
  const added=arrivingCard(latest?.game,next.game);if(added)arriving={id:added.id,at:Date.now()};
  if(latest?.game&&next.game&&latest.game.id===next.game.id&&['dealing','closing'].includes(latest.game.phase)&&!['dealing','closing'].includes(next.game.phase)){recalled=new Map(displayedDeclarations(latest.game).find(row=>row.seat===viewer)?.cards.map(card=>[card.id,Date.now()])||[]);}
  latest=next;
  ...
  if(lastDecision!==key){handDrag?.cancel();selected.clear();anchor=null;lastDecision=key;}
  selected=new Set([...selected].filter(id=>latest.game?.hand.some(card=>card.id===id)));render();
}
async function refreshState(){const next=await fetch('/api/state?seat='+viewer).then(response=>response.json());if(!next.csrf)throw new Error(t('本地服务暂不可用'));receive(next);return next;}
function connect(){
  stream?.close();connected=false;
  const connection=new EventSource('/api/events?seat='+viewer+'&client='+clientId+'&visible='+(viewMode==='game'&&!document.hidden));stream=connection;
  connection.onmessage=event=>{if(stream!==connection)return;const reconnect=!connected;connected=true;receive(JSON.parse(event.data));if(reconnect)presence();};
  connection.onerror=async()=>{if(stream!==connection)return;connected=false;render();if(!reconnecting){reconnecting=true;try{await refreshState();if(stream===connection)connect();}catch{}finally{reconnecting=false;}}};
}
function canPlay(){const game=latest?.game;return viewMode==='game'&&connected&&!submitting&&!latest?.paused&&!latest?.autoplay.includes(viewer)&&game?.pending?.seat===viewer&&['lead','follow','bury'].includes(game.pending.phase)&&!document.querySelector('dialog[open]');}
...
function handPanel(game){
  ...
  const handoff=game.seats.filter(seat=>seat.kind==='human').length>1&&decision&&game.seats[decision.seat].kind==='human'&&viewer!==decision.seat;
  if(handoff)return `<div class="hand-panel curtain"><h3>${pick('请把屏幕交给 ','Pass the screen to ')+escape(playerName(game,decision.seat))}</h3><p>${pick('准备好后再查看自己的手牌。','Reveal your own hand when you are ready.')}</p><button class="primary" id="handoff">${t('查看我的手牌')}</button></div>`;
  const mine=connected&&!submitting&&decision?.seat===viewer&&!latest.paused&&!latest.autoplay.includes(viewer);
  const playing=mine&&['lead','follow','bury'].includes(decision.phase);
  let prompt='',help='',bids='',actions='';
  if(bidding){
    prompt=latest.paused?pick('牌局已暂停','Game paused'):game.phase==='closing'?pick('最后反主时间','Last chance to counterdeclare'):pick('正在发牌','Dealing')+' · '+game.dealt+'/100';
    if(!latest.paused&&viewer>=0&&!latest.autoplay.includes(viewer))bids=game.bidOptions.map(item=>bidButton(item,game)).join('');
    help=pick('25 张手牌 · 庄家随后拿起 8 张底牌','25 cards each · the dealer then picks up the eight-card kitty');
  }else if(mine&&decision.phase==='declare'){prompt=(game.phase==='closing'?pick('最后亮主机会','Last declaration'):pick('轮到你亮主','Your declaration'))+' · '+game.dealt+'/100';bids=`<button class="secondary" data-bid="pass">${t('本次不亮')}</button>`+decision.options.map(item=>bidButton(item,game)).join('');}
  else if(mine&&decision.phase==='rebel'){prompt=pick('是否重新发牌？','Request a redeal?');bids=`<button class="primary" id="acceptRebel">${t('重新发牌')}</button><button class="secondary" id="declineRebel">${t('继续打')}</button>`;}
  else if(playing){
...
function layoutHand(){
  const board=$('cardTable'),frame=$('gameArea');
  if(fittingTable||handDrag?.active||!board||!frame||viewMode!=='game')return;
  const padding=getComputedStyle(document.body);
  $('gameView').style.height=Math.max(1,innerHeight-parseFloat(padding.paddingTop)-parseFloat(padding.paddingBottom))+'px';
  ...
  try{
    ...
    for(let pass=0;pass<5;pass++){
      board.style.width=(width/scale)+'px';board.style.left='0px';board.style.top='0px';
      board.style.transform=`scale(${scale})`;board.dataset.sceneScale=String(scale);
      layoutHandCards();
      const layout=layoutTable(height/scale);if(!layout)break;
      const fit=fitScene(width,height,board.offsetWidth,layout.height);
      if(fit.scale>=scale-.0005||pass===4){
        board.style.transform=`scale(${fit.scale})`;board.dataset.sceneScale=String(fit.scale);
        board.style.left=fit.left+'px';board.style.top=fit.top+'px';break;
      }
      scale=fit.scale;
    }
...
function layoutHandCards(){
  const root=$('playerHand'),content=$('handContent');if(!root||!content||!root.clientWidth||handDrag?.active)return;
  const nodes=[...content.querySelectorAll('.hand-slot')],layout=handLayout(nodes.length,root.clientWidth,appearance.handSize);
  content.style.width=layout.contentWidth+'px';$('cardTable').style.setProperty('--hand-h',layout.cardHeight+'px');root.style.setProperty('--card-w',layout.cardWidth+'px');root.style.setProperty('--card-h',layout.cardHeight+'px');root.dataset.rows='1';
  nodes.forEach((node,index)=>{const p=layout.positions[index];node.style.left=p.left+'px';node.style.bottom='0px';node.style.width=p.width+'px';node.style.height=layout.cardHeight+'px';node.style.setProperty('--bob-delay',index*-.17+'s');});
}
function layoutTable(sceneHeight){
  const board=$('cardTable'),probe=$('tableCardSize');if(!board||!probe?.offsetWidth)return;
  const narrow=false,north=board.querySelector('.seat.north');
  const sides=[...board.querySelectorAll('.seat.west,.seat.east')],panel=board.querySelector('.hand-panel');
  const spectatorSeat=latest?.game?.viewer<0?board.querySelector('.seat.south'):null;
  const gap=narrow?12:18,sideHeight=Math.max(0,...sides.map(node=>node.offsetHeight));
  const hudBottom=Math.max(...[...board.querySelectorAll('.trump-marker,.score-ticket')].map(node=>node.offsetTop+node.offsetHeight));
  const cardWidth=probe.offsetWidth,captionHeight=$('tableCaptionSize').offsetHeight;
  const layout=tableLayout({width:board.clientWidth,height:Math.max(850,sceneHeight),northBottom:north.offsetTop+north.offsetHeight,
    sideEdge:Math.max(...sides.map(node=>node.classList.contains('west')?node.offsetLeft+node.offsetWidth:board.clientWidth-node.offsetLeft)),sideHeight,hudBottom,
    handHeight:spectatorSeat?.offsetHeight||panel?.offsetHeight||0,handBottom:parseFloat(getComputedStyle(spectatorSeat||panel).bottom)||0,cardWidth,captionHeight,narrow});
  board.style.height=layout.height+'px';board.style.minHeight=layout.minimumHeight+'px';
  if(spectatorSeat)board.style.setProperty('--spectator-note-width',Math.max(70,board.clientWidth/2-spectatorSeat.offsetWidth/2-(narrow?10:22)-gap)+'px');
  for(const [key,value]of Object.entries({'north-y':layout.northTop,'side-y':layout.sideTop,'south-y':layout.southTop,'west-x':layout.west,'east-x':layout.east,'side-seat-y':layout.sideSeatTop,'hand-top':layout.handTop,'fan-max':layout.maxFanWidth}))board.style.setProperty('--'+key,value+'px');
  for(const node of board.querySelectorAll('.played-slot')){
    const count=node.querySelectorAll('.flip-card').length,badge=node.querySelector('.fan-overflow');
    node.style.setProperty('--fan-overlap',fanOverlap(cardWidth,count,layout.maxFanWidth-(badge?badge.offsetWidth+4:0),narrow?.5:.25)+'px');
  }
  const center=board.querySelector('.deck-center'),burial=board.querySelector('.burial-guide');
  for(const node of [center,burial])if(node){
    const northDeclared=board.querySelector('.declaration-pile.north')?.offsetHeight||0,southDeclared=board.querySelector('.declaration-pile.south')?.offsetHeight||0;
    const top=layout.northTop+northDeclared,bottom=layout.handTop-southDeclared-gap;
    node.style.top=Math.max(top,(top+bottom-node.offsetHeight)/2)+'px';node.style.transform='translateX(-50%)';
  }
  const drop=$('dropTarget');if(drop){drop.style.top=Math.max(layout.northTop,layout.handTop-160-gap)+'px';drop.style.bottom='auto';drop.style.height=Math.min(160,layout.handTop-layout.northTop-gap)+'px';}
  for(const node of board.querySelectorAll('.declaration-pile.south')){node.style.top=(layout.handTop-node.offsetHeight-gap)+'px';node.style.bottom='auto';}
  const boardRect=board.getBoundingClientRect(),scale=Number(board.dataset.sceneScale)||1;
  const flying=board.querySelector('.deal-flight'),stack=center?.querySelector('.deck-stack');
  if(flying&&stack&&!latest.paused&&hasLiveDealFlight(latest?.game,latest?.dealClock,Date.now())){
    const game=latest.game,position=relativePosition(game,game.drawSeat);
    const target=position==='south'&&$('playerHand')?.clientWidth?$('playerHand'):board.querySelector('.seat.'+position+' .backs');
    if(target){const from=stack.getBoundingClientRect(),to=target.getBoundingClientRect();
      flying.style.left=((from.left+from.width/2-boardRect.left)/scale)+'px';flying.style.top=((from.top+from.height/2-boardRect.top)/scale)+'px';
      flying.style.setProperty('--fly-x',((to.left+to.width/2-from.left-from.width/2)/scale)+'px');flying.style.setProperty('--fly-y',((to.top+to.height/2-from.top-from.height/2)/scale)+'px');
  ...
  for(const node of board.querySelectorAll('.played-slot[data-winner]')){
    const fan=node.querySelector('.played-fan'),target=node.dataset.winner==='south'&&$('playerHand')?.clientWidth?$('playerHand'):board.querySelector('.seat.'+node.dataset.winner+' .backs');
    if(!target)continue;
    const to=target.getBoundingClientRect(),from=node.getBoundingClientRect();
    ...
    node.style.setProperty('--collect-y',((to.top+to.height/2-boardRect.top)/scale-node.offsetTop-fan.offsetHeight/2)+'px');
  }
  return layout;
}
function bindHand(){
...
function render(){
  renderMenu();applyAppearance();document.body.dataset.screen=viewMode;const game=latest?.game;
  ...
  $('pauseGame').onclick=pauseGame;$('bookButton').onclick=()=>{$('bookDialog').showModal();renderBook();};$('lessonButton').onclick=()=>{$('lessonDialog').showModal();renderTraining(latest.game);};
  if($('resumeInline'))$('resumeInline').onclick=resumeGame;
  if($('nextDeal'))$('nextDeal').onclick=nextDeal;if($('resultMenu'))$('resultMenu').onclick=mainMenu;
  if($('handoff'))$('handoff').onclick=()=>{viewer=game.pending.seat;selected.clear();quiz=null;quizKey=null;connect();};
  ...
  if($('acceptRebel'))$('acceptRebel').onclick=()=>act({type:'rebel',accept:true});if($('declineRebel'))$('declineRebel').onclick=()=>act({type:'rebel',accept:false});
...
function renderBook(){
  if(!latest)return;const game=latest.game;
  renderNotebook(game,$('notebook'));renderStats();renderArchives();
  const labels={declaration:t('亮主'),pass:t('不亮'),throw_failed:t('甩牌未成立，改出最小组件'),rebel_choice:t('重发选择'),trump_set:t('定主'),buried:t('扣底'),deal_started:t('新一局')};
  const from=game?.events.findLast(event=>event.type==='deal_started')?.seq??-1;
  const records=game?.events.filter(event=>event.seq>=from&&(labels[event.type]||event.type==='trick'))||[];
  patchHtml($('history'),records.length?records.slice(-80).reverse().map(event=>`<div class="history-row"><span class="number">${event.type==='trick'?String(event.index+1).padStart(2,'0'):'·'}</span><span>${event.type==='trick'?escape(playerName(game,event.winner))+pick(' 收下 · ',' wins · ')+(game.tricks[event.index]?.plays.map(play=>playerName(game,play.seat)+' '+play.cards.map(cardLabel).join(' ')).map(escape).join(' / ')||''):(event.seat!==undefined?escape(seatName(game.seats[event.seat]))+' · ':'')+labels[event.type]+(event.type==='declaration'?' '+(event.suit?SYMBOLS[event.suit]:t('无主')):'')}</span>${event.type==='trick'?`<span class="points">+${event.points}</span>`:''}</div>`).join(''):`<p class="fine">${pick('发牌后，这里会记录本局的公开事件。','Public events appear here as the deal begins.')}</p>`);
  for(const tab of ['notebook','history','metrics'])$(tab).hidden=tab!==bookTab;
  document.querySelectorAll('[data-book-tab]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.bookTab===bookTab)));
}
function renderArchives(){const rows=latest?.archives||[];if(!rows.length)return;$('metrics').insertAdjacentHTML('beforeend',`<details class="archives"><summary>${pick('此前对局用量','Previous game usage')}</summary>${rows.slice().reverse().map(row=>`<div class="archive-row">${escape(row.at.slice(0,16).replace('T',' '))} · ${row.requests} ${pick('次请求','requests')}<br>${row.input} / ${row.output} tokens · $${row.cost.toFixed(6)} · ${pick('未知用量','unknown usage')} ${row.unknown}</div>`).join('')}</details>`);}
function renderStats() {
  if (latest?.statsDeferred) { $('metrics').innerHTML = t('<p class="fine">本局用量、耗时与代打记录将在结束后公开，避免泄露谁有机会叫主。后台仍完整记录每次调用。</p><p class="fine">本桌请求上限 ') + (latest.config?.limits.maxRequests ?? 100) + t(' 次 · 单次决策最多 12 秒</p>'); return; }
  const stats = latest?.stats || {};
  $('metrics').innerHTML = '<div class="stat-grid">' +
    [[t('真实 API 请求'), stats.realRequests || 0, t('次')], [t('离线模拟决策'), stats.simulated || 0, t('次')], [t('已报告输入 / 输出'), (stats.input || 0).toLocaleString() + ' / ' + (stats.output || 0).toLocaleString(), 'tokens'], [t('API 请求平均耗时'), stats.latencyCount ? Math.round(stats.mean) : '—', 'ms'], [t('API 请求中位耗时'), stats.latencyCount ? Math.round(stats.p50) : '—', 'ms'], [t('API 请求 P95 耗时'), stats.latencyCount ? Math.round(stats.p95) : '—', 'ms']].map(([label, value, unit]) => '<div class="stat"><small>' + label + '</small><strong>' + value + '</strong> <em>' + unit + '</em></div>').join('') +
    t('</div><div class="audit-status">最近 ') + (stats.latencyCount || 0) + t(' 次有耗时记录的 API 请求，含失败与超时<br>') + (latest?.game ? t('无需决策而跳过 ') + latest.game.forcedPasses + t(' 次亮主机会<br>') : '') + t('异常 ') + (stats.errors || 0) + t(' · 陪练代打 ') + (stats.fallbacks || 0) + t(' · 用量未知 ') + (stats.usageUnknown || 0) + t(' · 已报告缓存输入 ') + (stats.cached || 0) + t(' · 缓存未知 ') + (stats.cacheUnknown || 0) +
    t('<br>已报告推理 ') + (stats.reasoning || 0) + t(' tokens（已含在输出中）') +
    (stats.referencePricedRequests ? t('<br>标准价参考 $') + Number(stats.referenceCostUsd || 0).toFixed(6) + ' · ' + stats.referencePricedRequests + t(' 次已计价请求（非账单；包含套餐参考价或自填价，未含缓存优惠）') : t('<br>费用：暂无可计价的实际响应')) + '</div>' +
    (latest?.logs.findLast((entry) => entry.fallback) ? t('<p class="fine warning">最近一次陪练代打：') + escape(errorText(latest.logs.findLast((entry) => entry.fallback).reason)) + '</p>' : '') +
    (latest?.logs.at(-1)?.error ? '<p class="fine warning">' + escape(errorText(latest.logs.at(-1).error)) + '</p>' : '') + (latest?.game?.score ? t('<p class="fine"><a href="/api/audit" download>导出用量记录</a></p>') : '');
}
function renderTraining(game) {
  ...
  if (!game?.tricks.length || viewer < 0 || game.viewer !== viewer) { $('training').innerHTML = t('<p class="fine">至少完成一墩后，练习会出现在这里。观战视角不进行私有信息练习。</p>'); return; }
  const key = game.id + ':' + game.match.round + ':' + game.tricks.length + ':' + viewer;
  if (!quiz || quizAnswered && quizKey !== key) { quiz = trainingQuestion(game, locale); quizKey = key; quizAnswered = false; quizSelection.clear(); }
  if (!quiz) { $('training').innerHTML = '<p class="fine">' + pick('暂时没有合适的新题，完成下一墩再看看。','No suitable new exercise yet. Check again after the next trick.') + '</p>'; return; }
  patchHtml($('training'), '<span class="quiz-title">' + escape(quiz.title) + '</span><p class="quiz-question">' + escape(quiz.question) + (quiz.multiple ? t('（可多选）') : '') + '</p><div class="quiz-options">' +
    ...
    (quizAnswered ? '<p class="quiz-answer">' + (JSON.stringify([...quizSelection].sort()) === JSON.stringify([...quiz.correct].sort()) ? t('答对了。') : t('答案：') + quiz.correct.map(escape).join(pick('、', ', ')) + pick('。', '. ')) + escape(quiz.explanation) + '</p>' : t('<div class="hand-actions"><button class="secondary" id="answerQuiz">检查答案</button><button class="quiet" id="skipQuiz">看解析</button></div>')));
...
function labels(){
  const labels={menuTitle:['八十分','EIGHTY'],menuTagline:['一副好牌，一位好搭档。','A good hand. A better partnership.'],rulesButton:['玩法规则','Rules'],settingsButton:['设置','Settings'],creditsButton:['制作与鸣谢','Credits'],settingsBack:['← 主菜单','← Main menu'],settingsTitle:['设置','Settings'],settingsScope:['座位与规则用于新对局；画面偏好在返回牌桌后生效。','Seats and rules apply to new games. Appearance preferences apply when you return to the table.'],seatSettingsTitle:['四席入座','Four seats'],appearanceTitle:['画面与手感','Look and feel'],fourColorLabel:['四色花色','Four-color suits'],fourColorHelp:['黑桃深蓝、红桃红、梅花绿、方块橙；花色形状始终清晰。','Spades navy, hearts red, clubs green, diamonds orange. Suit shapes stay distinct.'],motionLabel:['轻微浮动','Gentle card motion'],motionHelp:['悬停平滑抬牌。系统的减少动态效果设置也会生效。','Cards lift smoothly on hover. Your system’s reduced-motion preference is respected.'],handLayoutHelp:['始终一排：25 张与拿底后的 33 张使用同一手牌区。牌桌会随窗口整体缩放。','Always one row: 25 cards and the dealer’s 33 cards share the same hand area. The complete table fits your window.'],apiSettingsTitle:['自己的 API','Your API'],apiSettingsHelp:['先连接服务并读取模型，再逐席分配。未启用时由陪练接手。','Connect a service, read its models, and assign them to individual seats. Inactive connections use a practice bot.'],learnSettingsTitle:['边玩边学','Learn as you play'],learnSettingsHelp:['在牌桌右上角随时开启。完成一墩后练习记牌与断门判断，不暂停牌局，也不调用 API。','Enable it from the table at any time. After a trick, practise card memory and proven voids without pausing play or making API calls.'],pauseTitle:['休息一下','Take a breath'],resumeGame:['继续游戏','Resume'],pauseRules:['玩法规则','Rules'],pauseMenu:['返回主菜单','Main menu'],newMatchTitle:['开始新的对局？','Start a new game?'],newMatchHelp:['当前对局将结束，双方从 2 开始，使用设置中安排的座位与规则。已经产生的 API 用量仍会保留。','This replaces the current game. Both teams start at 2 using your configured seats and rules. Existing API usage remains recorded.'],cancelRestart:['取消','Cancel'],confirmRestart:['开始新对局','New game'],bookTitle:['记牌簿','Notebook'],lessonTitle:['边玩边学','Learn as you play'],learnToggleLabel:['开启练习','Enable exercises'],lessonHelp:['完成一墩后出现一小题。可以回答、看解析或直接关掉；它不会改变分数。','A short exercise becomes available after a trick. Answer, inspect the explanation or close it; scoring is unaffected.'],skipLesson:['回到牌桌','Back to the table'],creditsTitle:['制作与鸣谢','Credits']};
  ...
  document.querySelectorAll('.setup-compass span').forEach((node,index)=>node.textContent=pick(['北','西','东','南'][index],['N','W','E','S'][index]));
  text('galleryButton','查看整副牌面','Card gallery');
  text('galleryTitle','整副牌面','Card gallery');
  text('settingsUsage','用量记录','Usage records');
  const tabs={notebook:['记牌','Card memory'],history:['牌桌记录','History'],metrics:['AI 用量','API usage']};document.querySelectorAll('[data-book-tab]').forEach(button=>button.textContent=pick(...tabs[button.dataset.bookTab]));
  $('creditsContent').innerHTML=`<div class="credits-list"><p>${pick('为四个人、两个搭档、一整晚的好牌而做。','For four players, two partnerships, and one more hand.')}</p><a class="credit-repo primary" href="https://github.com/tonyyunyang/80-fen-shengji" target="_blank" rel="noreferrer">Eighty · GitHub ↗</a><a class="credit-author" href="https://github.com/tonyyunyang" target="_blank" rel="noreferrer">Tony Yunyang ↗</a><hr><p>${pick('感谢 ChannonTian 提供的灵感。','Thanks to ChannonTian for the inspiration.')}</p><p>${pick('感谢 GPT6-Astra 的优质 token。','Thanks to GPT6-Astra for the excellent tokens.')}</p><p class="fine">${pick('视觉灵感：Balatro / LocalThunk。像素插画由 AI 生成，交互与引擎为 Eighty 实现。','Visual inspiration: Balatro / LocalThunk. AI-generated pixel artwork; interaction and engine by Eighty.')}</p><p class="fine">${pick('陪练参考源码保留 Apache-2.0 许可，详情见仓库。','The preserved practice strategy retains its Apache-2.0 license; details in the repository.')}</p></div>`;
}
$('rulesDialog').insertAdjacentHTML('beforeend',detailedRules());
$('menuArt').innerHTML=[{suit:'S',rank:11},{suit:'H',rank:12},{suit:'D',rank:13},{suit:'X',rank:16}].map(mini).join('');
$('appearancePreview').innerHTML=[{suit:'S',rank:13},{suit:'H',rank:12},{suit:'C',rank:14},{suit:'D',rank:10}].map(mini).join('');
labels();applyAppearance();
$('galleryButton').onclick=()=>{$('galleryContent').innerHTML=['S','H','C','D'].map(suit=>`<section><h3>${SYMBOLS[suit]}</h3><div class="card-gallery">${[14,13,12,11,10,9,8,7,6,5,4,3,2].map(rank=>mini({suit,rank})).join('')}</div></section>`).join('')+`<div class="card-gallery">${[15,16].map(rank=>mini({suit:'X',rank})).join('')}</div>`;$('galleryDialog').showModal();};$('closeGallery').onclick=()=>$('galleryDialog').close();
$('settingsUsage').onclick=()=>{bookTab='metrics';renderBook();$('bookDialog').showModal();};
$('menuStart').onclick=()=>{renderSetup();showScreen('setup');};$('setupBack').onclick=()=>showScreen('menu');$('continueGame').onclick=resumeGame;$('settingsButton').onclick=()=>{renderSetup();showScreen('settings');};$('settingsBack').onclick=()=>showScreen('menu');
$('rulesButton').onclick=()=>{rulesReturn='menu';$('rulesDialog').showModal();};$('creditsButton').onclick=()=>$('creditsDialog').showModal();
$('closeCredits').onclick=()=>$('creditsDialog').close();$('closeBook').onclick=()=>$('bookDialog').close();$('closeLesson').onclick=$('skipLesson').onclick=()=>$('lessonDialog').close();$('closeTrick').onclick=()=>$('trickDialog').close();
$('pauseRules').onclick=()=>{rulesReturn='pause';$('pauseDialog').close();$('rulesDialog').showModal();};$('closeRules').onclick=()=>$('rulesDialog').close();$('rulesDialog').addEventListener('close',()=>{if(rulesReturn==='pause'&&viewMode==='game'&&!$('pauseDialog').open)$('pauseDialog').showModal();});
$('resumeGame').onclick=resumeGame;$('pauseMenu').onclick=mainMenu;$('pauseDialog').addEventListener('cancel',event=>{event.preventDefault();$('pauseDialog').close();resumeGame();});
$('autoplay').onclick=async()=>{try{await post('autoplay',{seat:viewer,enabled:!latest.autoplay.includes(viewer)});await resumeGame();}catch(error){notify(error.message);}};
$('closeRestart').onclick=$('cancelRestart').onclick=()=>$('restartDialog').close();$('confirmRestart').onclick=()=>startMatch(true);
...
window.visualViewport?.addEventListener('resize',layoutHand);
document.addEventListener('visibilitychange',()=>{presence();applyAppearance();render();});motionPreference.addEventListener('change',()=>{applyAppearance();render();});
document.addEventListener('keydown',event=>{
  ...
  if(event.target.matches('.hand-slot')&&['ArrowLeft','ArrowRight','Home','End'].includes(event.key)){
    ...
    if(!cards[index])return;event.preventDefault();const id=Number(cards[index].dataset.card);if(event.shiftKey){setSelection(selectRange(latest.game.hand,selected,anchor??Number(event.target.dataset.card),id));}cards[index].focus();cards[index].scrollIntoView({block:'nearest',inline:'nearest'});return;
  }
