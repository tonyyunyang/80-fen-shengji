...
export function dealMarkers(game) {
  ...
  return `<div class="trump-marker" id="dealInfo" role="group" aria-label="${escape(facts.suitLabel+' · '+facts.suitName)}"><div class="trump-tile" data-suit="${facts.suit||'X'}"><strong>${facts.rank===null?'—':rankLabel(facts.rank)}</strong><span>${facts.suit?SYMBOLS[facts.suit]:facts.settled?'◇':'?'}</span></div><span class="trump-copy"><b>${facts.suitName}</b><small>${facts.settled?pick('级牌与王都算主','Level cards and jokers are trump'):facts.suitLabel}</small></span>${kitty}</div>`+
    `<div class="score-ticket ${ownDefends?'defending':'attacking'}"><b class="team-role">${facts.dealer===null?pick('等待定庄','Dealer undecided'):game.viewer<0?pick('攻守记分','Team score'):ownDefends?pick('我方 · 防守','WE DEFEND'):pick('我方 · 攻击','WE ATTACK')}</b><span>${game.viewer<0||facts.dealer===null?pick('攻方得分','Attacker points'):ownDefends?pick('对方攻分','Opponent points'):pick('我方攻分','Our points')}</span><strong>${game.score?.total??game.attackPoints} <small>/ 80</small></strong><span>${ownDefends?pick('守住 80 分以下','Keep them below 80'):pick('拿到 80 分上台','Reach 80 to take over')}</span></div>`;
}
...
export function trickMarkup(game,motion,narrow=false) {
  ...
  return plays.map((play,order)=>{
    ...
    return `<div class="played-slot ${position}" data-position="${position}" data-key="trick-${tableScope(game)}:${index}:${play.seat}" data-phase="${motion?.phase||'play'}"${motion?` data-winner="${relativePosition(game,motion.trick.winner)}"`:''}><button class="played-fan" data-review-trick="${index}" aria-label="${escape(seatLabel(game,play.seat)+' · '+type+' · '+play.cards.map(cardLabel).join(' '))}">`+
      ...
      (visible.length<play.cards.length?`<span class="fan-overflow">+${play.cards.length-visible.length}</span>`:'')+`</button><span class="play-caption"><span class="play-seat">${escape(playerName(game,play.seat))}</span><span> · ${order===0?t('领出'):t('跟牌')} · ${cardCount(play.cards.length)}</span></span></div>`;
  }).join('');
}
...
export function resultMarkup(game) {
  ...
  return `<section class="round-result" aria-label="${pick('本局结算','Round result')}"><span class="eyebrow">${pick('第','DEAL ')} ${game.match.round} ${pick('局结束','COMPLETE')}</span><h2>${headline}</h2><div class="result-score">${game.score.total}<small>${pick('攻方总分','attacker points')}</small></div><p>${game.score.attackPoints} ${pick('牌面分','trick points')} + ${game.score.kittyPoints} × ${kittyTaken?game.score.multiplier:0} ${pick('底牌','kitty')} = ${game.score.total}</p><p>${pick('南北','South / North')} ${levelLabel(game.match.levels[0])} · ${pick('东西','East / West')} ${levelLabel(game.match.levels[1])}</p><div class="result-kitty">${game.kitty.map(mini).join('')}</div><div class="dialog-actions">`+
