...
| Location | Owns |
| --- | --- |
| This repository / `main` | Shared rules, controllers, practice policy, pixel table, music, mobile controls, Node/BYOK app and optional hosting adapters. |
| This repository / `codex/tonytheyang-site` | Maintained production deployment channel, using the explicit `production` environment and its operator settings. |
| `tonytheyang.com` | Project link and promotional cover artwork. It does not vendor the game. |

The shared improvements through `44641b1` are included in `main` with
their history intact. Develop future shared improvements for `main`, then
merge the reviewed `main` into this branch before deploying. This branch may
match `main` exactly after a release; it remains the production channel.
Keep operator secrets in Cloudflare and keep production settings explicitly
scoped to `env.production` in `wrangler.jsonc`. Local `npm start` remains the
normal practice/BYOK app; sponsorship is disabled in the base Worker preview.
Do not delete this branch during merged-feature cleanup. Play the public game
at **[eighty.tonytheyang.com](https://eighty.tonytheyang.com/)**.

For production updates, switch to `codex/tonytheyang-site`, merge the verified
`origin/main`, push that branch, run `npm run deploy:production:check`, then
`npm run deploy:production`. Check the active Cloudflare version and public
asset hashes afterward. Publishing `main` on GitHub does not itself deploy
Cloudflare. The production domain and D1 binding belong to Tony's account;
forks must configure their own resources before deploying.
