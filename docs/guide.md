...
在「设置 → 光影与特效」里，可以选「华丽」的流动背景、牌面微光和得分火花，「柔和」的轻量提示，或「关闭」以使用静态背景。单独关闭「动态效果」可以让牌桌静止，系统的减少动态效果设置也会生效。「复古屏幕纹理」控制扫描线。背景音乐和牌桌音效默认开启，首次点击或触摸后播放；两者可分别调节音量和静音，已保存的静音选择会保留。
