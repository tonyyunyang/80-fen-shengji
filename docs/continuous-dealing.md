...
Status: implemented, updated 2026-09-13. New tables with zero or one human use continuous dealing. The default clock is 250ms per card; 500ms and 700ms remain available. Closing lasts at least five seconds. Local tables default to one human and three practice bots; the hosted edition supplies its configured models. Existing checkpoints retain their saved pace, and multi-human shared-screen games retain ordered mode.
...
1. **An independent deal clock.** The normal pace is 250ms per card: approximately 25 seconds for 100 cards and one new card in each player's hand every second. The earlier 500ms default remains an explicit comfortable option, alongside 700ms. An old setup using the former default migrates once to 250ms; subsequent explicit choices stay selected. The pace never changes because a player can bid, is thinking, passes, or times out. Mechanical draw ticks remain separate from player proposals.
