...
Node sessions become eligible for cleanup after 30 minutes without activity, checked once per minute. A visible connected page keeps its session active. Cleanup clears keys and pauses the saved game. Restarting the server also clears all keys. Node checkpoints retain game state and non-secret connection settings under ignored `data/browser-sessions/`; filenames hash the opaque cookie, and files use mode 0600. Hosted saves not used for seven days are removed. Local saves are retained. Keeping or restoring the browser cookie is necessary to resume its checkpoint.

The website's native Workers adapter uses SQLite checkpoints with the same metadata-only rule. With `PERSONAL_CONNECTIONS_ENABLED=true`, personal keys live only in their table object's memory. A timer keeps that object resident while keys are present; this consumes Durable Object active-duration quota even while paused. After 30 minutes without a public browser request, keys clear and the game pauses. Private model/bidding work does not extend this deadline. A Worker restart/deployment can also clear keys; saved personal connections then need their key again. Host connections continue to use Worker Secrets through a separate fixed-endpoint broker.
...
Base URLs reject embedded credentials, queries and fragments. Normal destinations require HTTPS and public addresses. IPv4, IPv6, mapped and reserved ranges are classified with ipaddr.js. The Node transport resolves each request, rejects non-public answers, and pins the validated address in the socket lookup. TLS certificate verification remains enabled. Redirects are never followed, response bodies are bounded to 1 MiB, and the existing decision deadline still applies. Browser JSON mutations are bounded to 128 KiB to accommodate discovered model metadata. The browser cannot provide arbitrary request headers or an arbitrary proxy body.

Workers does not support a custom HTTPS socket lookup, so its separate transport does **not** claim DNS pinning. It checks fresh A/AAAA answers on every request and uses `global_fetch_strictly_public` platform fetch. This deployment has no VPC, Tunnel, or private-network binding; do not add one without reassessing this boundary. Only model discovery and the three structured-action endpoints are accepted, and redirects are rejected before credentials can move. See [Workers HTTPS compatibility](https://developers.cloudflare.com/workers/runtime-apis/nodejs/https/), [DNS compatibility](https://developers.cloudflare.com/workers/runtime-apis/nodejs/dns/), and [public fetch routing](https://developers.cloudflare.com/workers/configuration/compatibility-flags/).
