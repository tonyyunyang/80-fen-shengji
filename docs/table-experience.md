...
Selected cards stay at a fixed raised height and do not join the hover wave, cursor tilt or idle bob. Selection and deselection use a critically damped transition that retains position and velocity during quick toggles. Their entire visible face is clickable, including the part overlapping the row below; stable target rectangles follow the painting order. A click retains the reading gap and binds repeated stationary contacts to the same physical card as it lifts. Moving the pointer, scrolling, resizing or changing keyboard focus releases that contact anchor. Native double-click pair selection is handled on the hand container because pointer capture can retarget the event there. These original transitions draw on the separation of hover, selection and drag states observed in the locally installed [Balatro](https://www.playbalatro.com/); no game code or assets are copied into this implementation.
...
At the end of a cancelled return, the hand adopts the card's last painted position and rotation, then settles toward any updated hover target. Moving the pointer across other cards therefore cannot cause a final jump. Pressing a returning card uses its visible position; only the re-grabbed members leave that return, while the others finish independently. Old animation callbacks cannot reveal or move cards owned by a newer drag. Returning-face geometry is read on a press or hand-back, never on continuous hover movement.

A just-selected card uses its settled selected height when recognizing an upward play gesture, so the required lift does not depend on animation progress. Its ghost still begins at the actual painted position. The minimum deliberate movement, field boundaries and complete-group validation continue to apply.
