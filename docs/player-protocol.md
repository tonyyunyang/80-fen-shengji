...
Continuous dealing is implemented for zero or one human. Shared-device multi-human games retain the ordered handoff mode. API seats use bilingual independent strategy v17; Chinese is the default, and English is selectable per seat. Legacy advice flags no longer affect normal play. The action protocol retains concurrent declaration bindings and private live-deal accounting. Context versions below describe retained evaluation profiles as well as current play.
