...
`partnershipRead` adds the complete accepted declaration timeline, exposed-card locations, public suit counts/leads, own short point holdings and retained structures. Overcalling does not erase earlier exposed cards. A nondealer's unplayed revealed card remains in that hand; a dealer's can be in the kitty. No new private bidding timing or pass signal is introduced. The event stream already stores these declarations, and completed replay tests verify that compression and later overcalls preserve them.

Possible counters remain the conservative v19 deductions below. Separately, a per-seat uniform-allocation reference conditions on its publicly allowed pool, hand size and fixed revealed cards. Single-card risks use exact combinatorics under that reference; pair/tractor counter packets use a union bound. A reference counter risk at most 0.04 can identify a likely point-feed window. This cutoff is a heuristic, not a calibrated confidence guarantee. Strategic burial, past choice bias and joint allocation weights are not modelled. Inconsistent counts or incomplete structural enumeration suppress the estimate. Uncertain outcomes never become `secured` because of this reference.

The guide compares point feeds and necessary entries, allows cashing an expendable weak pair, and considers strong side-suit leads that let a void partner discard. A single-card opportunity never certifies a pair lead. Explicit reminders retain caution for proven voids and economical support on zero-point tricks. A proven 80-point win overrides conservation reminders. All legal actions remain available and the model's returned action is not silently replaced.
