...
Shared game improvements and hosting adapters belong on `main`; merge the
reviewed `main` into `codex/tonytheyang-site` for production releases. The two
branches may share a commit after deployment. Preserve the deployment branch
even when merged. Production settings are explicit in `env.production` and
credentials remain in Worker Secrets.

Keep the public play link, https://eighty.tonytheyang.com/, visible in both
READMEs, the repository homepage and release notes. Historical release notes
must distinguish the continuously updated hosted game from their tagged code.
