...
- The pixel game at `public/index.html` is the only runtime interface. Keep configuration outside play, use one hand row for every phase, and preserve stable hover. Click selects in hand; a legal single-card drop plays immediately, while an incompatible drop returns to the hand. Cards and controls remain accessible HTML with reduced-motion support. Do not add alternate table or hand-lab entry points.
